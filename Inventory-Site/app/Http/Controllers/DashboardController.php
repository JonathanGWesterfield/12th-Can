...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $transactions = DashboardController::combineTransactions($transactions);
        $transactionNames = array();
        for ($i = 0; $i < count($transactions); ++$i) {
            $transactionNames[] = DashboardController::getItemName($transactions[$i]->item_id);
        }
        return view('pages.dashboard')->with('activeItems', $activeItems)->with('transactions', $transactions)->with('transactionNames', $transactionNames);
    }
}
